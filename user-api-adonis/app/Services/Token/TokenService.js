 ...
 * @returns {string}
 */
const generateToken = (userId, expires, secret = "thisisasimplesecret") => {
  const payload = {
    sub: userId,
    iat: moment().unix(),
    exp: expires.unix(),
...
const verifyToken = async (token, type) => {
  const payload = jwt.verify(token, "thisisasimplesecret");
...
const generateAuthTokens = async (user) => {
  const accessTokenExpires = moment().add("24", "minutes");
